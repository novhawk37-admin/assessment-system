...
async def submit_task(
    ...
    return {
        ...
        "upload_image": task.upload_image
    }
...
def list_tasks(
    ...
    if status_filter:
        query = query.filter(
            models.Task.status == status_filter
        )
...
def create_task(
    ...
    task = models.Task(
        ...
        status=payload.status or models.TaskStatus.assigned,
    )
    ...
    db.add(
        models.Activity(
            description=f"New task '{payload.title}' created by Admin",
...
def update_task(
    ...
    if (
        ...
        db.add(
            models.Activity(
                description=(
                    f"User '{task.assignee.name if task.assignee else 'Unknown'}' "
